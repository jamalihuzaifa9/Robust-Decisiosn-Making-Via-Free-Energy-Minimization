...
U_space_1 = np.linspace(-0.5, 0.5, control_space_size)
U_space_2 = np.linspace(-0.5, 0.5, control_space_size)
...
# URL to the GP model
url = r"https://github.com/GIOVRUSSO/Control-Group-Code/raw/refs/heads/master/Robust%20Decision-Making%20Via%20Free%20Energy%20Minimization/Experiments/GP_nominal.dump"  # <-- replace with actual link

# Download the file
response = requests.get(url)
response.raise_for_status()  # will raise an error if the download failed

# Load the GP model from the downloaded bytes
GP_nominal = pickle.load(BytesIO(response.content))
...
def Control_step(state, U_space_1, U_space_2, goal_points, obs_points):
    ...
    for i in range(control_space_size):
        for j in range(control_space_size):
            ...
            p_max = st.multivariate_normal(state.reshape((2,)),np.diag([1.0,1.0]))
            
            q = st.multivariate_normal(next_state_reference.reshape((2,)), cov_reference)
           
            reference_pdf = q.pdf(next_sample) + 1e-10
            reference_prob = reference_pdf / np.sum(reference_pdf)  # Avoid division by zero
            ...
            #                                 state.reshape((2,)),np.diag([1.0,1.0]))
...
M = 1  # Number of simulation runs (or experiments)

# Initial conditions for each simulation run (modify as needed)
# %% Evaluate Policy on Multiple Initial Conditions
initial_conditions = [np.array(np.mat('1.30;0.8; 0')),np.array(np.mat('0.5;-0.2; 0')),np.array(np.mat('1.2;-0.5; 0')),np.array(np.mat('-0.5;0.25; 0')),
...
for I in range(M):
    ...
    control_times = []  # Store control computation times per step
    ...
    while np.size(at_pose(np.vstack((x_si, x[2, :])), goal_points, position_error=0.25, rotation_error=100)) != N:
        ...
        control_times.append(t1 - t0)  # Store elapsed time for this step
    ...
    print(f"[Run {I+1}] Average Control Computation Time: {avg_control_time:.6f} seconds")
    
    average_control_times.append(avg_control_time)
...
# np.save(r'State_Data_Simulation_FPD_Pmax_210.npy', XX)
# np.save(r'Input_Data_Simulation_FPD_Pmax_210.npy', UU)
# # np.save(r'State_Data_nom.npy', XN)
# # np.save(r'COV_Data_nom.npy', COVN)

print("\n=== Summary of Average Control Times ===")
for idx, t in enumerate(average_control_times):
    print(f"Run {idx + 1}: {t:.6f} seconds")

# Optionally, save to file
np.save("avg_control_times.npy", np.array(average_control_times))
