...
from maxdiff_utils import Lambda
...
class KnownRewardModel(nn.Module):
    _activations = {
        "sin": Lambda(lambda x: torch.sin(x)),
        "relu": nn.ReLU(),
        "silu": nn.SiLU(),
    ...
    def __init__(self, num_states, num_actions, reward_fn,
                 init_w=3e-3, model_layers=[128, 128],
                 model_AF='relu', std=1e-6, stoch=False,
                 log_std_min=-10, log_std_max=2,
                 print_nets=False):

        super(KnownRewardModel, self).__init__()
        self.num_states  = nn.Parameter(torch.tensor(num_states), requires_grad=False)
        self.num_actions = nn.Parameter(torch.tensor(num_actions), requires_grad=False)
        self.stoch = stoch
        self.log_std_min = log_std_min
    ...
    def compute_reward(self, s, a):
        return torch.tensor([self.reward_fn(s[i].cpu().numpy(), a[i].cpu().numpy())
                            for i in range(s.shape[0])], device=s.device).unsqueeze(1)
