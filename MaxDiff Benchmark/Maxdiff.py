...
def state_cost(state,goal):
    return  1.0*state**2 

class SimpleMaxDiff:
    def __init__(self, model_fn, cost_fn, state_dim, action_dim, horizon=10, samples=20, lam=1.0,
                 alpha=0.1, eps=0.3, gamma=1.0,goal=None):
        self.H = horizon
        self.N = samples
        self.d_x = state_dim
        self.d_u = action_dim
        ...
        self.gamma = gamma
        self.goal = goal
        self.model_fn = model_fn  # dynamics: s_next = f(s, a)
        self.cost_fn = cost_fn
        # Nominal action plan: H x d_u
        self.a = np.zeros((self.H, self.d_u))

    def rollout(self, s0):
        # Sample N trajectories with noise
        ...
        for t in range(self.H):
            for i in range(self.N):
                ...
                rewards[t, i] = -self.cost_fn(s[t+1,i])  # simple reward: minimize state norm
        ...
        for i in range(self.N):
            traj = s[1:, i]  # drop s0
            cov = np.cov(traj.T) + 1e-5 * np.eye(self.d_x)
            entropy[i] = 0.5 * np.log(np.linalg.det(cov))

        R += self.alpha * entropy
        ...
        # Update nominal actions
        for t in range(self.H):
            update = np.sum(w[:, None] * eta[t], axis=0)
            self.a[t] += update

        return self.a[0]  # execute first action
